...
const cleanEncodingIssues = (text) => {
  ...
  return (
    text
      ...
      .replace(/ÃÂ/g, "À")
      .replace(/ÃÂ/g, "Ç")
      // NE PAS nettoyer les doubles espaces car ça peut affecter les nouvelles lignes
      .trim()
  );
};
...
export const githubAPI = {
  ...
  async createFileIfNotExists(filePath, initialContent) {
    try {
      ...
      if (response.status === 404) {
        ...
        const content = safeBase64Encode(
        ...
        if (!createResponse.ok) {
          throw new Error(
            `Erreur lors de la création du fichier: ${createResponse.status}`
          );
        }
  ...
  async uploadFile(file, targetPath) {
    try {
      ...
      if (file.name !== sanitizedFileName) {
        console.log(
          `⚠️ Nom de fichier sanitisé: "${file.name}" -> "${sanitizedFileName}"`
        );
      }
      ...
      const uploadPayload = {
        message: `Upload ${
          file.type.includes("image") ? "image" : "audio"
        }: ${sanitizedFileName}${
          file.name !== sanitizedFileName
            ? ` (sanitized from: ${file.name})`
            : ""
        }`,
      ...
      if (!uploadResponse.ok) {
        ...
        throw new Error(
          `Erreur upload: ${uploadResponse.status} - ${
  ...
  async deleteFile(filePath) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Erreur lors de la récupération du fichier: ${response.status}`
        );
      }
      ...
      if (!deleteResponse.ok) {
        throw new Error(
          `Erreur lors de la suppression: ${deleteResponse.status}`
        );
      }
  ...
  async updateCitationsFile(citations) {
    try {
      const response = await fetch(
        `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/src/data/citationsLibrary.json`,
        {
          headers: {
            Authorization: `token ${GITHUB_TOKEN}`,
            Accept: "application/vnd.github.v3+json",
      ...
      if (!updateResponse.ok) {
        throw new Error(
          `Erreur lors de la mise à jour du fichier: ${updateResponse.status}`
        );
      }
  ...
  async updatePhotosFile(photos) {
    try {
      const response = await fetch(
        `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/src/data/photoLibrary.json`,
        {
          headers: {
            Authorization: `token ${GITHUB_TOKEN}`,
            Accept: "application/vnd.github.v3+json",
      ...
      if (!updateResponse.ok) {
        throw new Error(
          `Erreur lors de la mise à jour du fichier photos: ${updateResponse.status}`
        );
      }
  ...
  async getPhotos() {
    try {
      ...
      let photos = await this.createFileIfNotExists(
        "src/data/photoLibrary.json",
        [
          {
            id: 1,
      ...
      photos = photos.map((photo) => {
        if (!photo.fullPath && photo.file) {
          ...
          console.log(
            `fullPath ajouté automatiquement pour ${photo.title}: ${photo.fullPath}`
          );
        }
        return photo;
      });
  ...
  async updateMusicFile(music) {
    try {
      const response = await fetch(
        `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/src/data/musicLibrary.json`,
        {
          headers: {
            Authorization: `token ${GITHUB_TOKEN}`,
            Accept: "application/vnd.github.v3+json",
      ...
      if (!updateResponse.ok) {
        throw new Error(
          `Erreur lors de la mise à jour du fichier musique: ${updateResponse.status}`
        );
      }
        ...
        if (photo.file && photo.file.includes(" ")) {
          ...
          try {
            ...
            const oldFileResponse = await fetch(
              `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/public/images/${oldFileName}`,
              {
                headers: {
                  Authorization: `token ${GITHUB_TOKEN}`,
                  Accept: "application/vnd.github.v3+json",
            ...
            if (oldFileResponse.ok) {
              ...
              await fetch(
                `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/contents/public/images/${newFileName}`,
                {
                  method: "PUT",
                  headers: {
                    Authorization: `token ${GITHUB_TOKEN}`,
            ...
            console.warn(
              `Erreur correction fichier ${oldFileName}:`,
              fileError
            );
          }
        }
